import { CheckCircleIcon, ClockIcon, PauseCircleIcon, PlayCircleIcon, XCircleIcon } from "@heroicons/react/16/solid";
...
export function getStatusConfig(status: CampaignStatus): {
	...
	const statusMap: Record<
		string,
		{ label: string; icon: typeof CheckCircleIcon; color: StatusColor; gradientClass: string }
	> = {
		active: {
			label: "Active",
			icon: PlayCircleIcon,
			color: "lime",
			gradientClass: "from-lime-500/20 via-lime-500/5 to-transparent dark:from-lime-500/15 dark:via-lime-500/5",
		},
		draft: {
			label: "Draft",
			icon: ClockIcon,
		...
		approved: {
			...
			color: "sky",
			gradientClass: "from-sky-500/20 via-sky-500/5 to-transparent dark:from-sky-500/15 dark:via-sky-500/5",
		},
		paused: {
			label: "Paused",
