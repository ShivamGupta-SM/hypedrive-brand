...
// =============================================================================
// WALLET + OVERVIEW HERO — Dark immersive card with key metrics
// =============================================================================
...
function HeroSection({
  ...
  return (
    <div className="relative overflow-hidden rounded-2xl bg-zinc-900 shadow-lg ring-1 ring-zinc-800 dark:bg-zinc-800/90 dark:ring-zinc-700">
      {/* Ambient glow effects */}
      <div className="pointer-events-none absolute -right-24 -top-24 size-72 rounded-full bg-emerald-500/10 blur-3xl" />
      <div className="pointer-events-none absolute -bottom-20 -left-12 size-56 rounded-full bg-violet-500/8 blur-3xl" />
      <div className="pointer-events-none absolute right-1/3 top-1/2 size-40 rounded-full bg-sky-500/6 blur-3xl" />

      <div className="relative p-5 sm:p-6">
        {/* Balance row */}
        <div className="flex items-start justify-between gap-4">
          <div className="min-w-0">
            <p className="text-[11px] font-medium uppercase tracking-wider text-zinc-400">Available Balance</p>
          ...
          <Link
            href={`/${orgSlug}/wallet`}
            className="shrink-0 rounded-lg bg-white/10 px-3.5 py-2 text-xs font-medium text-white backdrop-blur-sm transition-all hover:bg-white/15 hover:shadow-lg"
          >
            Manage Wallet
          </Link>
        </div>
        ...
        <div className="mt-6 grid grid-cols-2 gap-4 border-t border-white/10 pt-5 sm:grid-cols-4">
          <div className="min-w-0">
            ...
            <p className="mt-1 text-base font-semibold tabular-nums text-zinc-200">
              {formatCurrency(
                stats.paymentMode === "post_submission"
                  ? stats.pendingCommitmentsDecimal || "0"
                  : stats.heldAmountDecimal
              )}
            </p>
          </div>
          <div className="min-w-0">
          ...
          <div className="min-w-0">
            <p className="text-[11px] text-zinc-500">Active Campaigns</p>
            <div className="mt-1 flex items-baseline gap-1.5">
              <p className="text-base font-semibold tabular-nums text-zinc-200">{stats.activeCampaigns}</p>
              <span className="text-[11px] text-zinc-500">/ {stats.totalCampaigns}</span>
            </div>
          </div>
          <div className="min-w-0">
            <p className="text-[11px] text-zinc-500">Approval Rate</p>
            <div className="mt-1 flex items-baseline gap-1.5">
              <p className="text-base font-semibold tabular-nums text-zinc-200">{approvalRate}%</p>
              {stats.enrollmentTrend !== 0 && (
                <span
                  className={`text-[11px] font-medium ${
                    stats.enrollmentTrend > 0 ? "text-emerald-400" : "text-red-400"
                  }`}
                >
                  {stats.enrollmentTrend > 0 ? "+" : ""}
...
// =============================================================================
// MINI STAT PILLS — Enrollment + Pending counts inline
// =============================================================================
  ...
  return (
    <div className="grid grid-cols-3 gap-3">
      <Link
        ...
        className="group flex items-center gap-3 rounded-xl bg-sky-50/70 px-3.5 py-3 ring-1 ring-sky-200/60 transition-all hover:ring-sky-300 hover:shadow-sm dark:bg-sky-950/20 dark:ring-sky-800/50 dark:hover:ring-sky-700"
      >
        <div className="flex size-8 items-center justify-center rounded-lg bg-sky-100 dark:bg-sky-900/40">
          <UserGroupIcon className="size-4 text-sky-600 dark:text-sky-400" />
        </div>
        <div className="min-w-0">
            ...
            {stats.enrollmentTrend !== 0 && (
              <span
                className={`ml-1 font-medium ${
                  stats.enrollmentTrend > 0 ? "text-emerald-600 dark:text-emerald-400" : "text-red-500"
                }`}
              >
                {stats.enrollmentTrend > 0 ? "+" : ""}
                {stats.enrollmentTrend}%
      ...
      <Link
        href={`/${orgSlug}/enrollments`}
        className="group flex items-center gap-3 rounded-xl bg-amber-50/70 px-3.5 py-3 ring-1 ring-amber-200/60 transition-all hover:ring-amber-300 hover:shadow-sm dark:bg-amber-950/20 dark:ring-amber-800/50 dark:hover:ring-amber-700"
      >
        ...
        <div className="min-w-0">
          <p className="text-lg font-semibold tabular-nums text-zinc-900 dark:text-white">{stats.pendingEnrollments}</p>
          <p className="text-[11px] text-zinc-500 dark:text-zinc-400">
            Pending
            {stats.overdueEnrollments > 0 && (
              <span className="ml-1 font-medium text-red-500">{stats.overdueEnrollments} overdue</span>
            )}
          </p>
        </div>
      </Link>
      ...
      <Link
        ...
        className="group flex items-center gap-3 rounded-xl bg-violet-50/70 px-3.5 py-3 ring-1 ring-violet-200/60 transition-all hover:ring-violet-300 hover:shadow-sm dark:bg-violet-950/20 dark:ring-violet-800/50 dark:hover:ring-violet-700"
      >
        <div className="flex size-8 items-center justify-center rounded-lg bg-violet-100 dark:bg-violet-900/40">
          <MegaphoneIcon className="size-4 text-violet-600 dark:text-violet-400" />
        </div>
        <div className="min-w-0">
          <p className="text-lg font-semibold tabular-nums text-zinc-900 dark:text-white">
            {stats.endingSoon > 0 ? stats.endingSoon : "0"}
          </p>
          <p className="text-[11px] text-zinc-500 dark:text-zinc-400">
            {stats.endingSoon > 0 ? "Ending Soon" : "All on track"}
          </p>
        </div>
      </Link>
    </div>
...
function EnrollmentTrendChart({
  ...
  return (
    <ContentCard padding="none" className="flex h-full flex-col overflow-hidden">
      ...
      <div className="flex-1 px-3.5 py-3 sm:px-4">
...
function TopCampaignsSection({
  ...
  return (
    <ContentCard padding="none" className="flex h-full flex-col overflow-hidden">
      ...
      <div className="flex-1 px-1 py-1 sm:px-2">
          ...
          {campaigns.slice(0, 5).map((campaign) => {
            ...
            return (
              <Link
                ...
                className="group flex items-center gap-3 rounded-lg px-2.5 py-2.5 transition-colors hover:bg-zinc-50 dark:hover:bg-zinc-800/40"
              >
                {campaign.listingImage ? (
                  <div className="size-9 shrink-0 overflow-hidden rounded-lg bg-zinc-100 p-1 ring-1 ring-zinc-200/80 dark:bg-zinc-800 dark:ring-zinc-700">
                    <FadeImage src={getAssetUrl(campaign.listingImage)} alt="" className="size-full object-contain" />
                  </div>
                ) : (
                  <div className="flex size-9 shrink-0 items-center justify-center rounded-lg bg-zinc-100 ring-1 ring-zinc-200/80 dark:bg-zinc-800 dark:ring-zinc-700">
                    <MegaphoneIcon className="size-4 text-zinc-500 dark:text-zinc-400" />
                  </div>
                )}
                <div className="min-w-0 flex-1">
                  <div className="flex items-center justify-between gap-2">
                    <h3 className="truncate text-sm font-medium text-zinc-900 dark:text-white">{campaign.name}</h3>
                    <Badge color={sc.color} className="shrink-0 text-[10px]!">
                      {sc.label}
                    </Badge>
                  </div>
...
function ActivityFeed({ organizationId, orgSlug }: { organizationId: string; orgSlug: string }) {
    ...
    return (
      <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-4">
        {[1, 2, 3, 4].map((i) => (
          <div
            key={i}
            className="flex items-center gap-3 rounded-lg px-3 py-3 animate-fade-in"
  ...
  if (activities.length === 0) {
    return (
      <div className="flex flex-col items-center rounded-xl bg-zinc-50 px-4 py-8 text-center dark:bg-zinc-800/30">
        <div className="flex size-10 items-center justify-center rounded-xl bg-zinc-100 dark:bg-zinc-800">
...
function DashboardSkeleton() {
  return (
    <div className="space-y-4 animate-in fade-in duration-300 sm:space-y-5">
      ...
      {/* Hero skeleton */}
      <div className="rounded-2xl bg-zinc-900 p-5 sm:p-6 dark:bg-zinc-800">
...
export function Dashboard() {
  ...
  const hasChart = dashboardData.enrollmentChart && dashboardData.enrollmentChart.length > 1;
  const hasTopCampaigns = dashboardData.topCampaigns.length > 0;
  const hasPendingReviews = dashboardData.pendingEnrollments.length > 0 || stats.pendingEnrollments > 0;
    ...
    <div className="animate-page-enter min-w-0 space-y-4 sm:space-y-5">
      ...
      {isNewUser ? (
        ...
        <>
          ...
          {/* ROW 1: Hero — Dark immersive wallet + key metrics */}
          <ErrorBoundary title="Failed to load stats" compact>
            <HeroSection stats={stats} orgSlug={orgSlug} />
          </ErrorBoundary>

          {/* ROW 2: Mini stat pills — Enrollments, Pending, Ending Soon */}
          <MiniStatRow stats={stats} orgSlug={orgSlug} />

          {/* ROW 3: Quick Actions */}
          <QuickActions orgSlug={orgSlug} />

          {/* ROW 4: Content — Adaptive layout based on available data */}
          {(hasChart || hasTopCampaigns || hasPendingReviews) && (
            <div className="grid gap-4 lg:grid-cols-5">
              {/* Chart takes 3/5 when present, otherwise campaigns/reviews split evenly */}
              {hasChart && (
                <div className="lg:col-span-3">
              ...
              <div className={`space-y-4 ${hasChart ? "lg:col-span-2" : "lg:col-span-5"}`}>
                {!hasChart && (hasTopCampaigns || hasPendingReviews) ? (
                  /* No chart: campaigns + reviews side by side */
                  ...
                  /* Chart present: stack campaigns + reviews in sidebar */
                  <>
                    {hasTopCampaigns && (
                      <ErrorBoundary title="Failed to load top campaigns" compact>
                        <TopCampaignsSection campaigns={dashboardData.topCampaigns} orgSlug={orgSlug} />
