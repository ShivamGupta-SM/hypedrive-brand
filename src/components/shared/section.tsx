/**
 * Section Component
 * Standardized content section with icon, title, and description
 */
        ...
        <div className="mb-6 flex items-start justify-between gap-4">
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              {description && (
                <Text className="mt-1 text-zinc-500">{description}</Text>
              )}
            </div>
          </div>
...
export function SectionGroup({ children, className, spacing = "md" }: SectionGroupProps) {
  return (
    <div className={clsx(groupSpacing[spacing], className)}>
      {children}
    </div>
  );
}
...
export function DetailList({ children, className, columns = 2 }: DetailListProps) {
  return (
    <dl className={clsx("grid gap-4", detailColumns[columns], className)}>
      {children}
    </dl>
  );
}
