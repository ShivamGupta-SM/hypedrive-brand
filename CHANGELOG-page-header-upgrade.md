...
Upgraded `page-header.tsx` from 12 unused exports (568 lines) to 25+ production-adopted exports. Absorbed capabilities from 4 dead shared/ files. Adopted `PageHeader` in 8 list pages, `DetailPageHeader` in 2 detail pages, migrated `Card` → `ContentCard` in 5 files. Deleted 6 dead files. Zero TypeScript errors, zero Biome lint errors.
...
### Card → ContentCard Migration
| File | Action | Description |
|------|--------|-------------|
| `src/pages/enrollments/show.tsx` | **Modified** | `Card` → `ContentCard` import + all JSX (~10 instances) |
| `src/pages/dashboard.tsx` | **Modified** | `Card` → `ContentCard` import + all JSX (~8 instances) |
| `src/pages/wallet/transaction.tsx` | **Modified** | `Card` → `ContentCard` import + JSX (1 instance) |
| `src/pages/support/index.tsx` | **Modified** | `Card` → `ContentCard` import + JSX (2 instances) |
| `src/pages/listings/show.tsx` | **Modified** | `Card` → `ContentCard` import + JSX (2 instances) |
...
| `src/routes/_app/$orgSlug/code-audit.tsx` | **Modified** | Updated imports to page-header. Removed Breadcrumbs, Section, StatsRow demos (deleted components). Updated page-header verdict to "Upgraded & adopted". |
...
| `src/pages/component-demo.tsx` | **Deleted** | Not a route, only demonstrated dead components (had self-documenting DELETE comment) |
| `src/routes/_app/component-demo.tsx` | **Deleted** | Route file for deleted component-demo page |
...
| `src/components/shared/card.tsx` | Trimmed — removed Card/CardHeader/CardBody/CardFooter/CardDivider/CardGrid (moved to page-header.tsx). Kept StatCard + MiniStat only. |
## Verification

- `tsc --noEmit` — 0 errors
- `npx biome check` — 0 lint errors (formatting auto-fixed)
- All 6 phases complete

---

## Appendix: Junk & Bloat Report (separate from this upgrade)
...
~~components/page-header.tsx	~568	Zero importers~~ ✅ FIXED — upgraded & adopted
...
~~components/shared/info-panel.tsx	~50	Zero importers~~ ✅ DELETED (absorbed into page-header)
~~components/shared/section.tsx	~253	Zero importers~~ ✅ DELETED (absorbed into page-header)
~~components/shared/stats-row.tsx	~340	Zero importers~~ ✅ DELETED
~~components/shared/page-container.tsx	~201	Zero importers~~ ✅ DELETED (absorbed into page-header)
